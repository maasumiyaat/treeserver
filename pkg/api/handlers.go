...
import (
    ...
    "github.com/yourusername/treeserver/internal/tree"
    "github.com/yourusername/treeserver/internal/trie"
)

// RegisterHandlers wires HTTP endpoints.
...
func handleAVLInsert(w http.ResponseWriter, r *http.Request) {
    ...
    var s int
    for rows.Next() {
        if err := rows.Scan(&s); err == nil { avl.Insert(s) }
    }
    var vi int
    fmt.Sscanf(v, "%d", &vi)
    avl.Insert(vi)
...
func handleTrieAutocomplete(w http.ResponseWriter, r *http.Request) {
    ...
    var uname string
    for rows.Next() {
        if err := rows.Scan(&uname); err == nil { tr.Insert(strings.ToLower(uname)) }
    }
...
func handleHeapNext(w http.ResponseWriter, r *http.Request) {
    ...
    for rows.Next() {
        var t tree.Task
        if err := rows.Scan(&t.ID, &t.TaskName, &t.Priority); err == nil { tasks = append(tasks, t) }
    }
    if len(tasks) == 0 { json.NewEncoder(w).Encode(map[string]string{"message":"no tasks"}); return }
...
func handleQuadtVisible(w http.ResponseWriter, r *http.Request) {
    // simple wrapper
    x := r.URL.Query().Get("x"); y := r.URL.Query().Get("y")
    wbox := r.URL.Query().Get("w"); hbox := r.URL.Query().Get("h")
    var xf, yf, wf, hf float64
    fmt.Sscanf(x, "%f",&xf); fmt.Sscanf(y, "%f",&yf); fmt.Sscanf(wbox, "%f",&wf); fmt.Sscanf(hbox, "%f",&hf)
